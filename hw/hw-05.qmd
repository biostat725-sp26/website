...
Researchers would like to understand the **a posteriori** correlation between the anemia probability between two communities across Sud-Kivu. Repeat **Exercise 3** using the posterior mean of $\rho$. Visualize the correlations from the prior and posterioron the same figure. How does the posterior correlation compare to the prior?
...
Researchers are worried about any locations where the posterior probability is greater than 0.4, $p_i = P(\pi(\mathbf{u}_i) > 0.4 | \mathbf{Y})$. In particular, any locations with $p_i$ greater than 0.5 $\left(h_i = 1(p_i > 0.5)\right)$ indicates a hot spot region where public health individuals would like to intervene. Visualize both the $p_i$ and $h_i$ and make a statement about where intervention is warranted.
