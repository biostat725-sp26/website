...
This assignment is due on **Thursday, March 24 at 11:45am.** To be considered on time, the following must be done by the due date:
...
-   Go to the [biostat725-sp26](https://www.github.com/biostat725-sp26) organization on GitHub. Click on the repo with the prefix **hw-04**. It contains the starter documents you need to complete the homework.
...
```{r, eval = FALSE}
dat_hw4 <- readRDS("dat-hw4.rds")
```

```{r, echo = FALSE}
dat_hw4 <- readRDS("/Users/sib2/Box Sync/Faculty/Education/biostat725-sp25/course-material/data/glaucoma/dat-hw4.rds")
```

## Exercises 1-6

Define $Y_{it}$ as the MD value for eye $i$ ($i = 1,\ldots,n$) and visit $t$ ($t = 1,\ldots, n_i$), and $X_{it}$ is the follow-up time, such that $X_{i1} = 0$. Researchers are interested in estimating the eye-specific slopes for our sample to identify which patients are progressing (i.e., slope less than 0). To estimate these eye-specific slopes, researchers would like to fit the following model:
...
Explore the correlation between eye-specific intercepts ($\beta_{0i}$) and slopes ($\beta_{1i}$). Create a scatter plot to examine their relationship. Does their appear to be a relationship? To help answer this question also examine the posterior distribution for $\rho$ and comment on how it relates to the scatter plot.
...
#| warning: false
#| message: false
n_eyes_ols <- length(unique(dat_hw4$eye))
ols <- matrix(nrow = n_eyes_ols, ncol = 4)
for (i in 1:n_eyes_ols) {
  dat <- dat_hw4[dat_hw4$eye == i, ]
  mod <- lm(mean_deviation ~ time, data = dat)
  res <- summary(mod)
  pvalue <- pt(coef(res)[, 3], mod$df, lower = TRUE)[2]
  intercept <- coef(res)[1, 1]
  slope <- coef(res)[2, 1]  
  ols[i, ] <- c(i, intercept, slope, pvalue)  
}
ols <- data.frame(ols)
colnames(ols) <- c("eye", "intercept", "slope", "p-value")
ggplot(ols, aes(x = slope)) + 
  geom_histogram() + 
  labs(x = "Eye-Specific Slopes from OLS",
       y = "Count")
```
