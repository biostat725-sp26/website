...
\end{align*}
where $\mathbf{x}_i = (\text{education}_i,\text{income}_i,\text{color}_i)$ and $\alpha^*$ is the centered intercept.
...
fit_icar <- sampling(icar, stan_data, iter = 5000)
...
The code to create a map of the observed diabetes proportion and the posterior mean are below. 
