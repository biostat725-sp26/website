...
This object is an `sf` data object. The relevant variables are `GEOID`, which contains a unique identifier of each county and `geometry`, which contains the boundary information. For analyzing our data, we will first process the diabetes variable and then merge it with the `sf` data object. We begin by preparing the data for merging.

```{r}
diabetes_data <- diabetes_data %>%
  select(GEOID = `Geography Id`, diabetes = `Diabetes (Adult Population), 2019`) %>%
  mutate(
...
We then merge the diabetes data with the `sf` data object.

```{r}
durham_merged <- durham_tracts %>%
  left_join(diabetes_data , by = "GEOID")
```
...
Define $Y_i$ as the percentage of adults with diabetes in census tract $i$, for $i = 1,\ldots,n$. There are $n = 57$ census tract in Durham county. We are interested in producing a map of adult diabetes that has been smoothed using an ICAR process. We want to fit the following model: 

\begin{align*}
Y_i &= \alpha + \theta_i + \epsilon_i, \quad \epsilon_i \stackrel{iid}{\sim} N(0,\sigma^2)\\
\boldsymbol{\theta} &\sim \text{ICAR}(\tau^2)\\
\alpha &\sim N(0, 4^2)\\
\sigma &\sim \text{Half-Normal}(0, 4^2)\\
...
<!--   node1 = node1, -->
<!--   node2 = node2, -->
<!--   Y = durham_model_data$diabetes / 100, -->
...
<!-- ```{r, eval = FALSE} -->
<!-- icar_ae <- stan_model(file = "icar_ae.stan") -->
<!-- ``` -->

<!-- ```{r, echo = FALSE, eval = FALSE} -->
<!-- icar_ae <- stan_model(file = "/Users/sib2/Box Sync/Faculty/Education/biostat725-sp25/repos/ae-08/icar_ae.stan") -->
<!-- fit_icar_ae <- sampling(icar_ae, stan_data, iter = 5000, control = list(adapt_delta = 0.999,  -->
<!--                                max_treedepth = 10, -->
<!--                                stepsize = 1)) -->
<!-- print(fit_icar_ae, pars = c("alpha", "sigma", "tau")) -->
<!-- traceplot(fit_icar_ae, pars = c("alpha", "sigma", "tau")) -->
<!-- save(fit_icar_ae, file = "/Users/sib2/") -->
<!-- ``` -->


<!-- ```{r} -->
<!-- Y_pred <- rstan::extract(fit_icar_ae, pars = "Y_pred")$Y_pred -->
<!-- ppd_mean <- apply(Y_pred, 2, mean) -->
<!-- ppd_sd <- apply(Y_pred, 2, sd) -->
<!-- ``` -->

<!-- Merge predictions back to the dataset. Extract GEOIDs of non-NA tracts (in model order) and -->
...
<!-- ```{r} -->
<!-- # Merge predictions back into the full spatial object -->
