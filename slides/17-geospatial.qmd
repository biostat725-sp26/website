...
$$Y_j(\mathbf{u}_i) = \alpha + \mathbf{x}_j(\mathbf{u}_i)\boldsymbol{\beta} + \theta(\mathbf{u}_i) + \epsilon_j(\mathbf{u}_i).$$
...
-   $\boldsymbol{\epsilon}(\mathbf{u}_i) = (\epsilon_1(\mathbf{u}_i),\ldots,\epsilon_{n_i}(\mathbf{u}_i))^\top$, where $\epsilon_j(\mathbf{u}_i) \stackrel{iid}{\sim} N(0,\sigma^2)$.
...
(3) Posterior distribution: $f(\boldsymbol{\theta}, \boldsymbol{\Omega} | \mathbf{Y})$
...
-   Work on [HW 04](https://biostat725-sp26.netlify.app/hw/hw-04), which is due before class next Tuesday.

-   Complete reading to prepare for next Thursday's lecture

-   Thursday's lecture: Scalable Gaussian Processes #1
