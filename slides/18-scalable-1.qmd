...
$$\mathbf{C} \approx \boldsymbol{\Phi} \mathbf{S} \boldsymbol{\Phi}^\top.$$ - No matrix inversion. - $\boldsymbol{\Phi}$ only depends on $B$ and the observed locations, can be pre-calculated. - Each MCMC iteration requires $\mathcal{O}(nm + m)$ flops, vs $\mathcal{O}(n^3)$ for a full GP. - Ideally $m \ll n$, but HSGP can be faster even for $m>n$.
...
We model $\boldsymbol{\theta} = \boldsymbol{\Phi} \mathbf{S}^{1/2}\mathbf{b}$, where $\mathbf{b}$ is treated as the unknown parameter. Therefore for kriging: $$\begin{align}
...
::: fragment
If $m>n$, do we need to invert an $n \times n$ matrix $(\boldsymbol{\Phi}\mathbf{S}\boldsymbol{\Phi}^\top)$ for kriging?
:::
