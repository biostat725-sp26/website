...
beta <- rnorm(3)
poly <- c("linear", "quadratic", "cubic")
df <- cbind.data.frame(
  time = rep(t, 3), 
  out = c(t * beta[1], t^2 * beta[2] + t * beta[1], t^3 * beta[3] + t^2 * beta[2] + t * beta[1]),
  type = rep( factor(poly, levels = poly), each = 50)
)
ggplot(df) + 
  geom_line(aes(x = time, y = out, group = type, colour = type)) +
  ylab("value")
```
...
## What do I want the function to look like? {.midi}
...
## What are my prior assumptions about the covariance? {.midi}

If I write that covariance as $C(t,s)$, then we must **decide** what the function $C$ looks like.

i.  *Hypothesis 1* : The marginal variability should stay the same.
...
## Combining GP with Random Effects Regression {.midi}
...
\right),
\end{align*} where $\mathbf{k}_i = \mathbb{C}(\mathbf{f}^{pred}_i, \mathbf{f}_i)$ ("cross-covariances").
...
i.  Do a bit of software research on [Wikipedia](https://en.wikipedia.org/wiki/Comparison_of_Gaussian_process_software)

ii. Code MCMC yourself to make it faster (e.g., using `Rcpp`)

iii. Scalable Approximations (Activee research area)
...
-   Work on [HW 04](https://biostat725-sp26.netlify.app/hw/hw-04-blank)
