...
-   The Bayesian LOO-CV estimate of out-of-sample predictive fit is $$\widehat{\text{elpd}}_{\text{LOO-CV}} = \sum_{i=1}^n \log f\left(Y_i|\mathbf{Y}_{-i}\right),$$
calculated as $$\sum_{i=1}^n \log \left(\frac{1}{S} \sum_{s=1}^S f\left(Y_i|\boldsymbol{\theta}^{(is)}\right)\right),\quad \boldsymbol{\theta}^{(is)} \sim f(\boldsymbol{\theta}|\mathbf{Y}_{-i}).$$
