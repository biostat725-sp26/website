...
functions {
  ...
  vector theta_new_rng(array[] vector x, array[] vector x_new, real tau, 
    ...
    vector[n_new] theta_new;
      ...
      matrix[n, n_new] Cplus = gp_matern32_cov(x, x_new, tau, rho);
      matrix[n, n_new] LCinv_Cplus = mdivide_left_tri_low(LC, Cplus);
      vector[n] LCinv_theta_obs = mdivide_left_tri_low(LC, theta_obs);
      vector[n_new] theta_new_mu = LCinv_Cplus' * LCinv_theta_obs;
      matrix[n_new, n_new] C_star = add_diag(gp_matern32_cov(x_new, tau, rho), delta);
      matrix[n_new, n_new] theta_new_cov = C_star - LCinv_Cplus' * LCinv_Cplus;
...
data {
  ...
  int<lower = 1> n_new;                    // total number of points for prediction
  ...
  array[n_new] vector[d] u_new;          // locations for prediction
  matrix[n_new, p] X_new;                 // design matrix for the new locations
}
transformed data {
  real delta = 1e-9;
...
generated quantities {
  ...
  vector[n_new] theta_new = theta_new_rng(u, u_new, tau, rho, z, delta);
  array[n_new] real Y_new = normal_rng(alpha + X_new * beta + theta_new, sigma);
